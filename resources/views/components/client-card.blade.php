<a href="{{ route('clients.show', $client->id) }}"
    class="{{ $client->status == 1 ? 'bg-orange-300' : ($client->waitlist == 1 ? 'bg-blue-300' : 'bg-green-300') }} min-h-content relative z-0 m-2 flex flex-col justify-between rounded-lg border border-blue-200 p-2 shadow-md shadow-blue-100 transition-all duration-200 ease-in hover:border hover:border-blue-500 hover:shadow-lg"
    style="z-index: 1;">
